...
class TurInfo extends \yii\base\Object
{
	...
	public function loadNames()
	{
            ...
            foreach ($this->inArrMore as $value) {
                if(isset($value['dancer1']) && array_search($value['dancer1'], $ids)===FALSE){
                    $ids[]=$value['dancer1'];
                }
                if(isset($value['dancer2']) && array_search($value['dancer2'], $ids)===FALSE) {
                    $ids[]=$value['dancer2'];
                }
            }
	...
	public function GetNameCouple($inId)
	{
            ...
            $nameCouple=[];
            if (isset($this->inArrMore[$inId]['dancer1']) && isset($this->names[$this->inArrMore[$inId]['dancer1']])) {
                $nameCouple[]=$this->names[$this->inArrMore[$inId]['dancer1']];
            }
            if (isset($this->inArrMore[$inId]['dancer2']) && isset($this->names[$this->inArrMore[$inId]['dancer2']])) {
                $nameCouple[]=$this->names[$this->inArrMore[$inId]['dancer2']];
            }
	...
	public function getTur($name) 
        {
            if (isset($this->tur[$name])){
                return $this->tur[$name];
            } else {
                return FALSE;
            }
	}
	...
	public function getDances()
		...
		$this->arrDance = array_fill_keys(explode(',',str_replace(' ','',$this->tur['dances'])), '');
                ...
                foreach ($Dance->each() as $row) {
                    if (isset($this->arrDance[$row['id']])){
                        $this->arrDance[$row['id']]=$row['name']; 
                    }
                }
            }
